...
function PrincipleItem({
  ...
  return (
    <motion.div ref={itemRef} style={{ opacity, y }} className="relative w-full">
      <div className="relative w-full h-px mb-10 md:mb-16">
        <div className="absolute inset-0 bg-white/10" />
        <motion.div style={{ width: lineWidth }} className="absolute inset-0 h-px bg-[#66FF80]/40" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-12 items-start">
        <div className="md:col-span-1">
          <span className="text-sm font-mono text-white/25 tracking-widest">{principle.number}</span>
        </div>
      ...
      {isLast && <div className="w-full h-px bg-white/10 mt-10 md:mt-16" />}
    </motion.div>
  );
}
...
export default function PrinciplesSection() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full bg-[#0a0a0a] text-white overflow-hidden py-32 md:py-48">
      <motion.div style={{ y: yParallax }} className="max-w-[1400px] mx-auto px-6 md:px-16 w-full">
